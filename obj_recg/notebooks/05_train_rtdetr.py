...
# MAGIC %md
# MAGIC # 05 · Train RT-DETR (transformer detector)
# MAGIC
# MAGIC RT-DETR is a detection transformer. It ships in Ultralytics with the same
# MAGIC API as YOLO, so it reuses the Ultralytics training path. Serverless GPU.
# MAGIC Detection only.

# COMMAND ----------

# Interactive runs only:
# %pip install "ultralytics>=8.3" "pycocotools>=2.0.7" "mlflow>=3.1.0"
...
for k, v in {"catalog": "rpeng_upleveling", "project": "seeing_models",
             ...
             "gpu_type": "A10", "model_family": "detr", "sport": "golf"}.items():
...
results = {}
for spec in specs_for(family=Family.DETR):
